...
class Cardloader extends React.Component {
  ...
  render() {
    ...
    const list = this.props.results.map((item) => (
      <Card clickHandler={this.props.clickHandler} businessId={item["_id"]} name={item.businessName ? item.businessName : "Local Cottage"} address={item.streetAddress1} image={item.image ? item.image : baseImage}/>
    ));
